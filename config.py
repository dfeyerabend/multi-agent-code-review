...
OPTIMIZER_PROMPT = (
    ...
    "## Your Tools\n"
    "- `generate_fix_suggestion`: Call this for each line in a finding's `lines` list. "
    "Pass the full `code` and that line number. "
    ...
    "  - `index`: this finding's position in the list — use this to reference the "
    "finding in `submit_optimization`. Never restate `rule` or `line` in your output.\n"
    "  - `rule`: the rule code that flagged this issue (e.g. 'B608', 'W291') — context "
    "for understanding the issue type, not part of your output\n"
    "  - `lines`: every line number this finding affects (1-based). Always use this "
    "list, never `line`, when deciding which lines to fix — `lines` is always complete, "
    "even when it has only one entry.\n"
    "  - `line`: convenience copy of `lines[0]`. Do not use this for fixing — read `lines` instead.\n"
    ...
    "1. For every line number in `lines`, call `generate_fix_suggestion` with the full "
    "`code` and that line. If a call returns `status: 'error'` for a line, note it and "
    "continue with the remaining lines.\n"
    "2. Using the returned context plus `rationale` and `best_practice_refs` (or "
    "`doc_url` if empty), write one `suggested_code` snippet and `explanation` that "
    "together resolve the issue at every line in `lines` — not only the first.\n"
    "   - If every call in step 1 errored, set `suggested_code` to null and record the "
    "error(s) in `explanation` instead.\n"
    "3. Populate `grounded_in` with the sources you relied on, "
    "using the format: 'pyguide §3.10' or 'company_rules §1.3' or the `doc_url` value. "
    "Example: [\"pyguide §2.1\", \"company_rules §1.4\"]\n"
    "4. Add the fix to your `submit_optimization` call as `{index, suggested_code, "
    "explanation, grounded_in}` — `index` must match this finding's `index`. "
    "Do not include `rule` or `line`/`lines` in the fix entry; they are attached automatically.\n"
    ...
    "`generate_fix_suggestion` errored for ALL of its lines.\n"
    "- Every fix entry must reference its finding's `index`. Never output `rule`, "
    "`line`, or `lines` in a fix entry — those are attached automatically from the original finding.\n"
    "- Always read line numbers from `lines`, never from `line` — `lines` is the complete, "
    "authoritative list, even when a finding has only one entry.\n"
    "- A fix must address every line in `lines`, not only the first.\n"
